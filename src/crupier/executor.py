...
from .errors import CrupierModelUnsupportedError, CrupierProviderUnavailableError, CrupierStructuredOutputError
...
class RouteExecutor:
    ...
    def _execute_tools(
        ...
        planning = self._call_model(model, planning_prompt, request, trace, raw_outputs, role="tool_planner")
        ...
        final_response = self._call_model(model, final_prompt, request, trace, raw_outputs, role="tool_final")
        output_json = self._parse_or_repair_structured(
            final_response.text,
            request,
            trace,
    ...
    def _execute_structured(
        ...
        for model in self._models_in_execution_order(plan):
            response = self._call_model(model, prompt, request, trace, raw_outputs, role="structured")
            try:
                ...
                repair = self._call_model(model, repair_prompt, request, trace, raw_outputs, role="structured_repair")
                try:
