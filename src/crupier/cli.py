...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"crupier {__version__}")
    parser.add_argument("--project", default=".", help="Project directory containing crupier.toml")
    parser.add_argument(
        "--env-file",
        help="Load missing provider environment variables from a local KEY=value file resolved relative to --project",
    )
    ...
    release_check.add_argument(
        "--verify-project-urls",
        action="store_true",
        help="Verify that public [project.urls] links are reachable before publishing",
    )
    ...
    serve.add_argument(
        "--allow-remote",
        action="store_true",
        help="Allow binding to non-loopback interfaces. Use only behind your own network/auth boundary.",
    )
    serve.add_argument(
        "--cors-origin",
        help="Opt in to browser CORS for a specific origin. Use '*' only in trusted local experiments.",
    )
    serve.add_argument(
        "--compat-mode",
...
def _load_env_file(path: str, *, project: str) -> dict[str, str]:
    env_path = Path(path)
    if not env_path.is_absolute():
        env_path = Path(project) / env_path
    env_path = env_path.resolve()
    if not env_path.exists():
        raise CrupierConfigError(
            f"Environment file not found: {env_path}",
            hint="Create a local ignored .env file or remove --env-file.",
        )
    loaded: dict[str, str] = {}
    for line_number, raw_line in enumerate(env_path.read_text(encoding="utf-8").splitlines(), start=1):
        parsed = _parse_env_file_line(raw_line, line_number=line_number)
        if parsed is None:
            continue
        key, value = parsed
        if key in os.environ:
            loaded[key] = "already-set"
            continue
...
def _parse_env_file_line(raw_line: str, *, line_number: int) -> tuple[str, str] | None:
    line = raw_line.strip()
    ...
    if "=" not in line:
        raise CrupierConfigError(f"Invalid environment file line {line_number}: expected KEY=value.")
    key, value = line.split("=", 1)
    key = key.strip()
    if not _ENV_FILE_KEY_RE.fullmatch(key):
        raise CrupierConfigError(f"Invalid environment variable name on line {line_number}.")
    return key, _parse_env_file_value(value.strip(), line_number=line_number)


def _parse_env_file_value(value: str, *, line_number: int) -> str:
    if not value:
        return ""
    if value[0] in {"'", '"'}:
        quote = value[0]
        if len(value) < 2 or value[-1] != quote:
            raise CrupierConfigError(f"Invalid quoted environment value on line {line_number}.")
        inner = value[1:-1]
        if quote == '"':
            inner = _decode_env_double_quoted(inner)
        return inner
    return _strip_env_inline_comment(value)


def _decode_env_double_quoted(value: str) -> str:
    replacements = {"n": "\n", "r": "\r", "t": "\t", "\\": "\\", '"': '"'}
    chars: list[str] = []
    index = 0
    while index < len(value):
        ...
        if char == "\\" and index + 1 < len(value):
            index += 1
            chars.append(replacements.get(value[index], value[index]))
        else:
            chars.append(char)
        index += 1
    return "".join(chars)
...
def cmd_release_check(args: argparse.Namespace) -> int:
    ...
    if args.verify_project_urls:
        report.checks.append(check_project_urls_reachable(args.project))
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    if args.cors_origin:
        print(f"Browser CORS enabled for origin: {args.cors_origin}", file=sys.stderr)
    try:
...
def _strict_public_release_check(report: Any) -> ReleaseCheck:
    ...
    failure_ids = [check.id for check in report.checks if check.status == "fail"]
    build_skipped = bool(report.build.get("skipped"))
    ok = not warning_ids and not failure_ids and not build_skipped
    actions: list[str] = []
    if failure_ids:
        actions.append("Resolve release failures before publishing publicly: " + ", ".join(failure_ids) + ".")
    if warning_ids:
    ...
    return ReleaseCheck(
        ...
        if ok
        else "Strict public release gate failed because release failures, warnings, or skipped build checks remain.",
        evidence={"failure_ids": failure_ids, "warning_ids": warning_ids, "build_skipped": build_skipped},
        actions=actions,
    )
...
_SECRET_REPLACERS = (
    ...
    (
        re.compile(r"(you\s+\()[A-Za-z0-9][A-Za-z0-9_\-]{3,63}(\))", re.IGNORECASE),
        r"\1[redacted]\2",
    ),
    (
        re.compile(
            r"(\bref:\s*)[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}",
            re.IGNORECASE,
        ),
        r"\1[redacted]",
    ),
    (
        re.compile(
            r"(\b(?:account|account_id|organization|org|project|user)\s*[:=]\s*)[A-Za-z0-9_\-]{4,64}",
            re.IGNORECASE,
        ),
        r"\1[redacted]",
    ),
