...
from .common import build_prompt, env_value
...
class OllamaAdapter:
    ...
    def generate(self, *, model: str, prompt: str, request: RequestEnvelope) -> AdapterResponse:
        ...
        if self._requires_cloud_auth() and not api_key:
            raise CrupierProviderAuthError(
                ...
                hint="Set OLLAMA_API_KEY for https://ollama.com/api, or configure host explicitly for a local Ollama daemon.",
            )
        if api_key:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=float(request.constraints.get("timeout_seconds", 120))) as response:
    ...
    def _probe_structured_output(self, *, model: str, request: RequestEnvelope) -> AdapterResponse:
        body = self._chat_json(
            ...
            timeout=float(request.constraints.get("timeout_seconds", 60)),
        )
        message = body.get("message", {}) if isinstance(body, dict) else {}
        text = message.get("content", "")
    ...
    def _requires_cloud_auth(self) -> bool:
        base = self._base_url()
        return "ollama.com" in base and not base.startswith("http://localhost") and not base.startswith("http://127.0.0.1")
    ...
    def _raise_http_error(self, exc: urllib.error.HTTPError) -> None:
        ...
        raise CrupierProviderUnavailableError(f"Ollama HTTP {exc.code}: {message}") from exc
