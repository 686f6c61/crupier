...
def schema_from_request(request: RequestEnvelope) -> dict[str, Any] | None:
    ...
    if isinstance(raw_schema, dict):
        response_format_schema = _schema_from_response_format(raw_schema)
...
def _schema_from_response_format(value: dict[str, Any]) -> dict[str, Any] | None:
    if value.get("type") == "json_object":
        return {"type": "object"}
    if value.get("type") != "json_schema":
        return None
    if isinstance(value.get("schema"), dict):
        return dict(value["schema"])
