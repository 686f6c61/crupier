...
def validate() -> list[str]:
    ...
    if len(parser.json_ld) != 1:
        ...
        try:
            ...
            if structured_data.get("version") != SITE_VERSION:
                fail(errors, f"JSON-LD version must be {SITE_VERSION}")
    ...
    if not parser.scripts:
        fail(errors, "site must load at least one external script")
    for source in parser.scripts:
        if not source:
            fail(errors, "inline executable scripts are not allowed")
        elif urlsplit(source).scheme and source not in ALLOWED_EXTERNAL_SCRIPTS:
            fail(errors, f"external script is not allowlisted: {source}")
